"use strict";
(self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] = self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] || []).push([["8898"], {
21545(__unused_rspack_module, __webpack_exports__, __webpack_require__) {
// ESM COMPAT FLAG
...
__webpack_require__.d(__webpack_exports__, {
  ...
  pasteURLAsLink: () => (/* binding */ pasteURLAsLink),
  insertNewlineContinueMarkup: () => (/* binding */ insertNewlineContinueMarkup),
  deleteMarkupBackward: () => (/* binding */ deleteMarkupBackward),
  insertNewlineContinueMarkupCommand: () => (/* binding */ insertNewlineContinueMarkupCommand)
});

// EXTERNAL MODULE: consume shared module (default) @codemirror/state@~6.5.4 (strict) (fallback: /home/runner/work/notebook/notebook/.jupyter_releaser_checkout/node_modules/@codemirror/state/dist/index.js)
var index_js_ = __webpack_require__(84473);
// EXTERNAL MODULE: consume shared module (default) @codemirror/view@~6.39.11 (strict) (fallback: /home/runner/work/notebook/notebook/.jupyter_releaser_checkout/node_modules/@codemirror/view/dist/index.js)
var dist_index_js_ = __webpack_require__(47604);
// EXTERNAL MODULE: consume shared module (default) @codemirror/language@^6.12.1 (strict) (fallback: /home/runner/work/notebook/notebook/.jupyter_releaser_checkout/node_modules/@codemirror/language/dist/index.js)
var language_dist_index_js_ = __webpack_require__(58540);
// EXTERNAL MODULE: ../node_modules/@codemirror/autocomplete/dist/index.js
var dist = __webpack_require__(73343);
// EXTERNAL MODULE: consume shared module (default) @lezer/common@~1.5.0 (strict) (fallback: /home/runner/work/notebook/notebook/.jupyter_releaser_checkout/node_modules/@lezer/common/dist/index.js)
var common_dist_index_js_ = __webpack_require__(3508);
...
function skipForList(bl, cx, line) {
    ...
    if (line.indent >= line.baseIndent + 4)
        return false;
    let size = (bl.type == dist_Type.OrderedList ? isOrderedList : isBulletList)(line, cx, false);
    return size > 0 &&
        (bl.type != dist_Type.BulletList || isHorizontalRule(line, cx, false) < 0) &&
        line.text.charCodeAt(line.pos + size - 1) == bl.value;
}
const DefaultSkipMarkup = {
    [dist_Type.Blockquote](bl, cx, line) {
        if (line.next != 62 /* '>' */)
            return false;
        line.markers.push(dist_elt(dist_Type.QuoteMark, cx.lineStart + line.pos, cx.lineStart + line.pos + 1));
        line.moveBase(line.pos + (dist_space(line.text.charCodeAt(line.pos + 1)) ? 2 : 1));
        bl.end = cx.lineStart + line.text.length;
        return true;
    },
    [dist_Type.ListItem](bl, _cx, line) {
        if (line.indent < line.baseIndent + bl.value && line.next > -1)
            return false;
        line.moveBaseColumn(line.baseIndent + bl.value);
        return true;
    },
    [dist_Type.OrderedList]: skipForList,
    [dist_Type.BulletList]: skipForList,
    [dist_Type.Document]() { return true; }
};
function dist_space(ch) { return ch == 32 || ch == 9 || ch == 10 || ch == 13; }
function skipSpace(line, i = 0) {
    while (i < line.length && dist_space(line.charCodeAt(i)))
        i++;
    return i;
}
function skipSpaceBack(line, i, to) {
    while (i > to && dist_space(line.charCodeAt(i - 1)))
        i--;
    return i;
}
...
function isBulletList(line, cx, breaking) {
    return (line.next == 45 || line.next == 43 || line.next == 42 /* '-+*' */) &&
        (line.pos == line.text.length - 1 || dist_space(line.text.charCodeAt(line.pos + 1))) &&
        (!breaking || inList(cx, dist_Type.BulletList) || line.skipSpace(line.pos + 2) < line.text.length) ? 1 : -1;
}
function isOrderedList(line, cx, breaking) {
    ...
    if (pos == line.pos || pos > line.pos + 9 ||
        (next != 46 && next != 41 /* '.)' */) ||
        (pos < line.text.length - 1 && !dist_space(line.text.charCodeAt(pos + 1))) ||
        breaking && !inList(cx, dist_Type.OrderedList) &&
...
function isSetextUnderline(line) {
    ...
    while (pos < line.text.length && line.text.charCodeAt(pos) == line.next)
        pos++;
    let end = pos;
    while (pos < line.text.length && dist_space(line.text.charCodeAt(pos)))
        pos++;
...
function addCodeText(marks, from, to) {
    ...
    if (last >= 0 && marks[last].to == from && marks[last].type == dist_Type.CodeText)
        marks[last].to = to;
    else
        marks.push(dist_elt(dist_Type.CodeText, from, to));
}
...
const DefaultBlockParsers = {
    ...
    IndentedCode(cx, line) {
        ...
        if (pendingMarks.length) {
            pendingMarks = pendingMarks.filter(m => m.type != dist_Type.CodeText);
            if (pendingMarks.length)
                line.markers = pendingMarks.concat(line.markers);
        }
        cx.addNode(cx.buffer.writeElements(marks, -from).finish(dist_Type.CodeBlock, to - from), from);
        return true;
    },
    FencedCode(cx, line) {
        ...
        let marks = [dist_elt(dist_Type.CodeMark, from, from + len)];
        if (infoFrom < infoTo)
            marks.push(dist_elt(dist_Type.CodeInfo, cx.lineStart + infoFrom, cx.lineStart + infoTo));
        for (let first = true; cx.nextLine() && line.depth >= cx.stack.length; first = false) {
            ...
            if (i - line.pos >= len && line.skipSpace(i) == line.text.length) {
                for (let m of line.markers)
                    marks.push(m);
                marks.push(dist_elt(dist_Type.CodeMark, cx.lineStart + line.pos, cx.lineStart + i));
                cx.nextLine();
                break;
            }
            else {
        ...
        cx.addNode(cx.buffer.writeElements(marks, -from)
            .finish(dist_Type.FencedCode, cx.prevLineEnd() - from), from);
        return true;
    },
    Blockquote(cx, line) {
        ...
        if (size < 0)
            return false;
        cx.startContext(dist_Type.Blockquote, line.pos);
        cx.addNode(dist_Type.QuoteMark, cx.lineStart + line.pos, cx.lineStart + line.pos + 1);
    ...
    HorizontalRule(cx, line) {
        ...
        cx.nextLine();
        cx.addNode(dist_Type.HorizontalRule, from);
        return true;
    },
    BulletList(cx, line) {
        ...
        if (size < 0)
            return false;
        if (cx.block.type != dist_Type.BulletList)
            cx.startContext(dist_Type.BulletList, line.basePos, line.next);
        let newBase = getListIndent(line, line.pos + 1);
        cx.startContext(dist_Type.ListItem, line.basePos, newBase - line.baseIndent);
        cx.addNode(dist_Type.ListMark, cx.lineStart + line.pos, cx.lineStart + line.pos + size);
    ...
    OrderedList(cx, line) {
        ...
        if (size < 0)
            return false;
        if (cx.block.type != dist_Type.OrderedList)
            cx.startContext(dist_Type.OrderedList, line.basePos, line.text.charCodeAt(line.pos + size - 1));
    ...
    ATXHeading(cx, line) {
        ...
        while (after > off && line.text.charCodeAt(after - 1) == line.next)
            after--;
        if (after == endOfSpace || after == off || !dist_space(line.text.charCodeAt(after - 1)))
            after = line.text.length;
        let buf = cx.buffer
            .write(dist_Type.HeaderMark, 0, size)
            .writeElements(cx.parser.parseInline(line.text.slice(off + size + 1, after), from + size + 1), -from);
        if (after < line.text.length)
            buf.write(dist_Type.HeaderMark, after - off, endOfSpace - off);
        let node = buf.finish(dist_Type.ATXHeading1 - 1 + size, line.text.length - off);
        cx.nextLine();
    ...
    HTMLBlock(cx, line) {
        ...
        if (trailing)
            cx.nextLine();
        let nodeType = end == CommentEnd ? dist_Type.CommentBlock : end == ProcessingEnd ? dist_Type.ProcessingInstructionBlock : dist_Type.HTMLBlock;
...
class LinkReferenceParser {
    ...
    complete(cx, leaf, len) {
        cx.addLeafElement(leaf, dist_elt(dist_Type.LinkReference, this.start, this.start + len, this.elts));
        return true;
    }
    nextStage(elt) {
        if (elt) {
    ...
    advance(content) {
        for (;;) {
            if (this.stage == -1 /* RefStage.Failed */) {
                ...
                this.elts.push(dist_elt(dist_Type.LinkMark, this.pos + this.start, this.pos + this.start + 1));
                this.pos++;
            }
...
class SetextHeadingParser {
    nextLine(cx, line, leaf) {
        ...
        if (underline < 0)
            return false;
        let underlineMark = dist_elt(dist_Type.HeaderMark, cx.lineStart + line.pos, cx.lineStart + underline);
        cx.nextLine();
        cx.addLeafElement(leaf, dist_elt(next == 61 ? dist_Type.SetextHeading1 : dist_Type.SetextHeading2, leaf.start, cx.prevLineEnd(), [
...
class BlockContext {
    ...
    ranges) {
        ...
        this.block = CompositeBlock.create(dist_Type.Document, 0, this.lineStart, 0, 0);
    ...
    readLine() {
        ...
        for (; line.depth < this.stack.length; line.depth++) {
            ...
            if (!handler)
                throw new Error("Unhandled block context " + dist_Type[cx.type]);
    ...
    finishLeaf(leaf) {
        ...
        this.addNode(this.buffer
            ...
            .finish(dist_Type.Paragraph, leaf.content.length), leaf.start);
    }
    elt(type, from, to, children) {
        if (typeof type == "string")
            return dist_elt(this.parser.getNodeType(type), from, to, children);
        ...
        if (nonEmpty(config.defineNodes)) {
            ...
            for (let s of config.defineNodes) {
                ...
                let group = composite ? ["Block", "BlockContext"] : !block ? undefined
                    : id >= dist_Type.ATXHeading1 && id <= dist_Type.SetextHeading2 ? ["Block", "LeafBlock", "Heading"] : ["Block", "LeafBlock"];
...
let dist_nodeTypes = [common_dist_index_js_.NodeType.none];
for (let i = 1, name; name = dist_Type[i]; i++) {
    dist_nodeTypes[i] = common_dist_index_js_.NodeType.define({
        id: i,
        name,
        props: i >= dist_Type.Escape ? [] : [[common_dist_index_js_.NodeProp.group, i in DefaultSkipMarkup ? ["Block", "BlockContext"] : ["Block", "LeafBlock"]]],
        top: name == "Document"
    });
}
const none = [];
...
function dist_elt(type, from, to, children) {
...
const DefaultInline = {
    Escape(cx, next, start) {
        ...
        for (let i = 0; i < Escapable.length; i++)
            if (Escapable.charCodeAt(i) == escaped)
                return cx.append(dist_elt(dist_Type.Escape, start, start + 2));
        return -1;
    },
    Entity(cx, next, start) {
        ...
        return m ? cx.append(dist_elt(dist_Type.Entity, start, start + 1 + m[0].length)) : -1;
    },
        ...
        for (; pos < cx.end; pos++) {
            if (cx.char(pos) == 96) {
                ...
                if (curSize == size && cx.char(pos + 1) != 96)
                    return cx.append(dist_elt(dist_Type.InlineCode, start, pos + 1, [
                        dist_elt(dist_Type.CodeMark, start, start + size),
                        dist_elt(dist_Type.CodeMark, pos + 1 - size, pos + 1)
                    ]));
            }
            else {
                curSize = 0;
    ...
    HTMLTag(cx, next, start) {
        ...
        if (url) {
            return cx.append(dist_elt(dist_Type.Autolink, start, start + 1 + url[0].length, [
                dist_elt(dist_Type.LinkMark, start, start + 1),
                // url[0] includes the closing bracket, so exclude it from this slice
                dist_elt(dist_Type.URL, start + 1, start + url[0].length),
                dist_elt(dist_Type.LinkMark, start + url[0].length, start + 1 + url[0].length)
            ]));
        }
        let comment = /^!--[^>](?:-[^-]|[^-])*?-->/i.exec(after);
        if (comment)
            return cx.append(dist_elt(dist_Type.Comment, start, start + 1 + comment[0].length));
        let procInst = /^\?[^]*?\?>/.exec(after);
        if (procInst)
            return cx.append(dist_elt(dist_Type.ProcessingInstruction, start, start + 1 + procInst[0].length));
        let m = /^(?:![A-Z][^]*?>|!\[CDATA\[[^]*?\]\]>|\/\s*[a-zA-Z][\w-]*\s*>|\s*[a-zA-Z][\w-]*(\s+[a-zA-Z:_][\w-.:]*(?:\s*=\s*(?:[^\s"'=<>`]+|'[^']*'|"[^"]*"))?)*\s*(\/\s*)?>)/.exec(after);
        if (!m)
            return -1;
        return cx.append(dist_elt(dist_Type.HTMLTag, start, start + 1 + m[0].length));
    },
    ...
    HardBreak(cx, next, start) {
        if (next == 92 /* '\\' */ && cx.char(start + 1) == 10 /* '\n' */)
            return cx.append(dist_elt(dist_Type.HardBreak, start, start + 2));
        if (next == 32) {
            let pos = start + 1;
            while (cx.char(pos) == 32)
                pos++;
            if (cx.char(pos) == 10 && pos >= start + 2)
                return cx.append(dist_elt(dist_Type.HardBreak, start, pos + 1));
        }
        return -1;
    },
    Link(cx, next, start) {
    ...
    LinkEnd(cx, next, start) {
        ...
        for (let i = cx.parts.length - 1; i >= 0; i--) {
            ...
            if (part instanceof InlineDelimiter && (part.type == LinkStart || part.type == ImageStart)) {
                ...
                let link = cx.parts[i] = finishLink(cx, content, part.type == LinkStart ? dist_Type.Link : dist_Type.Image, part.from, start + 1);
...
function finishLink(cx, content, type, start, startPos) {
    ...
    content.unshift(dist_elt(dist_Type.LinkMark, start, start + (type == dist_Type.Image ? 2 : 1)));
    content.push(dist_elt(dist_Type.LinkMark, startPos - 1, startPos));
    if (next == 40 /* '(' */) {
        ...
        if (cx.char(pos) == 41 /* ')' */) {
            content.push(dist_elt(dist_Type.LinkMark, startPos, startPos + 1));
            endPos = pos + 1;
            if (dest)
                content.push(dest);
            if (title)
                content.push(title);
            content.push(dist_elt(dist_Type.LinkMark, pos, endPos));
        }
    }
    else if (next == 91 /* '[' */) {
    ...
    return dist_elt(type, start, endPos, content);
}
...
function parseURL(text, start, offset) {
    ...
    if (next == 60 /* '<' */) {
        for (let pos = start + 1; pos < text.length; pos++) {
            ...
            if (ch == 62 /* '>' */)
                return dist_elt(dist_Type.URL, start + offset, pos + 1 + offset);
            if (ch == 60 || ch == 10 /* '<\n' */)
                return false;
        }
        return null;
        ...
        return pos > start ? dist_elt(dist_Type.URL, start + offset, pos + offset) : pos == text.length ? null : false;
    }
}
function parseLinkTitle(text, start, offset) {
    ...
    for (let pos = start + 1, escaped = false; pos < text.length; pos++) {
        ...
        if (escaped)
            ...
            return dist_elt(dist_Type.LinkTitle, start + offset, pos + 1 + offset);
        else if (ch == 92 /* '\\' */)
            escaped = true;
    }
    return null;
...
function parseLinkLabel(text, start, offset, requireNonWS) {
    for (let escaped = false, pos = start + 1, end = Math.min(text.length, pos + 999); pos < end; pos++) {
        ...
        if (escaped)
            ...
            return requireNonWS ? false : dist_elt(dist_Type.LinkLabel, start + offset, pos + 1 + offset);
        else {
            if (requireNonWS && !dist_space(ch))
                requireNonWS = false;
            if (ch == 91 /* '[' */)
                return false;
            else if (ch == 92 /* '\\' */)
...
const NotLast = [dist_Type.CodeBlock, dist_Type.ListItem, dist_Type.OrderedList, dist_Type.BulletList];
    ...
    takeNodes(cx) {
        ...
        for (;;) {
            ...
            if (cur.to - off <= cx.ranges[cx.rangeI].to) { // Fits in current range
                ...
                let dummy = new common_dist_index_js_.Tree(cx.parser.nodeSet.types[dist_Type.Paragraph], [], [], 0, cx.block.hashProp);
...
const dist_parser = new MarkdownParser(new common_dist_index_js_.NodeSet(dist_nodeTypes).extend(markdownHighlighting), Object.keys(DefaultBlockParsers).map(n => DefaultBlockParsers[n]), Object.keys(DefaultBlockParsers).map(n => DefaultLeafBlocks[n]), Object.keys(DefaultBlockParsers), DefaultEndLeaf, DefaultSkipMarkup, Object.keys(DefaultInline).map(n => DefaultInline[n]), Object.keys(DefaultInline), []);
...
function parseCode(config) {
    ...
    let wrap = (0,common_dist_index_js_.parseMixed)((node, input) => {
        let id = node.type.id;
        if (codeParser && (id == dist_Type.CodeBlock || id == dist_Type.FencedCode)) {
            let info = "";
            if (id == dist_Type.FencedCode) {
                let infoNode = node.node.getChild(dist_Type.CodeInfo);
                if (infoNode)
                    info = input.read(infoNode.from, infoNode.to);
            }
            let parser = codeParser(info);
            if (parser)
                return { parser, overlay: node => node.type.id == dist_Type.CodeText };
        }
        else if (htmlParser && (id == dist_Type.HTMLBlock || id == dist_Type.HTMLTag)) {
...
function dist_count(str, from, to, ch) {
    let result = 0;
    for (let i = from; i < to; i++)
        if (str[i] == ch)
            result++;
...
function autolinkURLEnd(text, from) {
    ...
    for (;;) {
        ...
        if (/[?!.,:*_~]/.test(last) ||
            last == ")" && dist_count(text, from, end, ")") > dist_count(text, from, end, "("))
            end--;
...
var lang_html_dist = __webpack_require__(19768);
...
const commonmark = /*@__PURE__*/dist_parser.configure({
    props: [
...
function mkLang(parser) {
    return new language_dist_index_js_.Language(data, parser, [], "markdown");
}
/**
...
function blankLine(context, state, line) {
    ...
    for (let i = 0, e = context.length - 2; i <= e; i++) {
        insert += context[i].blank(i < e
            ? (0,index_js_.countColumn)(line.text, 4, context[i + 1].from) - insert.length
            : null, i < e);
    }
...
function htmlTagCompletions() {
    ...
    let result = (0,lang_html_dist/* .htmlCompletionSource */.$g)(new dist/* .CompletionContext */._5(index_js_.EditorState.create({ extensions: htmlNoMatch }), 0, true));
...
*/
const pasteURLAsLink = /*@__PURE__*/dist_index_js_.EditorView.domEventHandlers({
    paste: (event, view) => {
        var _a;
        ...
        if (!markdownLanguage.isActiveAt(view.state, main.from, 1))
            return false;
        let tree = (0,language_dist_index_js_.syntaxTree)(view.state), crossesNode = false;
...
//# sourceMappingURL=8898.733a4b358e3198f6.js.map?v=733a4b358e3198f6