"use strict";
(self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] = self["webpackChunk_JUPYTERLAB_CORE_OUTPUT"] || []).push([["175"], {
57490(__unused_rspack_module, __webpack_exports__, __webpack_require__) {
// ESM COMPAT FLAG
...
__webpack_require__.d(__webpack_exports__, {
  ...
  BlockType: () => (/* binding */ dist_BlockType),
  BidiSpan: () => (/* binding */ BidiSpan),
  Direction: () => (/* binding */ dist_Direction),
  ...
  BlockWrapper: () => (/* binding */ BlockWrapper),
  WidgetType: () => (/* binding */ WidgetType),
  panels: () => (/* binding */ dist_panels),
  ...
  tooltips: () => (/* binding */ dist_tooltips),
...
var style_mod = __webpack_require__(66910);
...
var crelt = __webpack_require__(13183);
...
*/
class BlockWrapper extends index_js_.RangeValue {
...
const dist_scrollIntoView = /*@__PURE__*/index_js_.StateEffect.define({ map: (t, ch) => t.map(ch) });
...
// Provide decorations
const dist_decorations = /*@__PURE__*/index_js_.Facet.define();
const dist_blockWrappers = /*@__PURE__*/index_js_.Facet.define();
...
class WidgetBufferTile extends Tile {
    ...
    get overrideDOMText() { return index_js_.Text.empty; }
...
class TileUpdate {
    constructor(view, old, blockWrappers, decorations, disallowBlockEffectsFor) {
        ...
        this.builder = new TileBuilder(this.cache, new DocTile(view, view.contentDOM), index_js_.RangeSet.iter(blockWrappers));
    ...
    emit(from, to) {
        ...
        let openEnd = index_js_.RangeSet.spans(this.decorations, from, to, {
    ...
    coordsForChar(pos) {
        ...
        function scan(tile, offset) {
            if (tile.isComposite()) {
                ...
                let end = (0,index_js_.findClusterBreak)(tile.text, offset);
                if (end == offset)
                    return null;
...
function moveVertically(view, start, forward, distance) {
    ...
    return index_js_.EditorSelection.cursor(pos.pos, pos.assoc, undefined, goal);
}
...
function posAtCoordsInline(view, tile, offset, x, y) {
    ...
    if (tile.isText()) {
        for (let i = 0; i < tile.length;) {
            let next = (0,index_js_.findClusterBreak)(tile.text, i);
            checkRects(textRange(tile.dom, i, next).getClientRects(), i);
            if (!dxClosest && !dyClosest)
                break;
            i = next;
        }
        let after = (x > (closestRect.left + closestRect.right) / 2) == (dirAt(view, closest + offset) == dist_Direction.LTR);
        return after ? new PosAssoc(offset + (0,index_js_.findClusterBreak)(tile.text, closest), -1) : new PosAssoc(offset + closest, 1);
    }
    else {
        if (!tile.length)
...
class DOMReader {
    constructor(points, view) {
        ...
        this.view = view;
        this.text = "";
        this.lineSeparator = view.state.facet(index_js_.EditorState.lineSeparator);
    }
    append(text) {
        this.text += text;
    }
    ...
    constructor(view, start, end, typeOver) {
        ...
        if (view.state.readOnly && start > -1) {
            ...
            if (view.inputState.composing > -1 && view.state.selection.ranges.length > 1)
                this.newSel = view.state.selection.replaceRange(index_js_.EditorSelection.range(anchor, head));
            else
...
function applyDefaultInsert(view, change, newSel) {
    ...
    if (inAtomic > -1) {
        tr = {
            changes: change,
            selection: index_js_.EditorSelection.cursor(change.from + change.insert.length, -1)
        };
    }
...
function heightRelevantDecoChanges(a, b, diff) {
    let comp = new dist_DecorationComparator;
    index_js_.RangeSet.compare(a, b, diff, comp, 0);
    return comp.changes;
}
class dist_DecorationComparator {
    constructor() {
        this.changes = [];
    }
    compareRange() { }
...
function staticDeco(state) {
    ...
    if (outer.length)
        deco.push(index_js_.RangeSet.join(outer));
    return deco;
}
    ...
    constructor(view) {
        ...
        this.handlers.textupdate = e => {
            ...
            let change = { from: diff.from + from, to: diff.toA + from,
                insert: index_js_.Text.of(e.text.slice(diff.from, diff.toB).split("\n")) };
...
class EditorView {
    ...
    */
    static scrollIntoView(pos, options = {}) {
        return dist_scrollIntoView.of(new ScrollTarget(typeof pos == "number" ? index_js_.EditorSelection.cursor(pos) : pos, options.y, options.x, options.yMargin, options.xMargin));
    }
    /**
    ...
    */
    scrollSnapshot() {
        ...
        return dist_scrollIntoView.of(new ScrollTarget(index_js_.EditorSelection.cursor(ref.from), "start", "start", ref.top - scrollTop, scrollLeft, true));
    }
    /**
...
function createDialog(view, config, result) {
    ...
    if (!content) {
        content = (0,crelt/* ["default"] */.A)("form");
        if (config.input) {
            let input = (0,crelt/* ["default"] */.A)("input", config.input);
            if (/^(text|password|number|email|tel|url)$/.test(input.type))
                input.classList.add("cm-textfield");
            if (!input.name)
                input.name = "input";
            content.appendChild((0,crelt/* ["default"] */.A)("label", (config.label || "") + ": ", input));
        }
        else {
            content.appendChild(document.createTextNode(config.label || ""));
        }
        content.appendChild(document.createTextNode(" "));
        content.appendChild((0,crelt/* ["default"] */.A)("button", { class: "cm-button", type: "submit" }, config.submitLabel || "OK"));
    }
    ...
    let panel = (0,crelt/* ["default"] */.A)("div", content, (0,crelt/* ["default"] */.A)("button", {
        onclick: () => done(null),
...
@internal
*/
const __test = { HeightMap, HeightOracle, MeasuredHeights, QueryType: dist_QueryType, ChangedRange, computeOrder,
...
//# sourceMappingURL=175.73524b900cb25d2d.js.map?v=73524b900cb25d2d